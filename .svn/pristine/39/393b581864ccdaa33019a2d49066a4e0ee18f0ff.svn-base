...
class Menu {
	...
	private function getNav($fid){
		...
		$field='id,nav_name,nav_color,english';
    ...
    public function getNavInfo($id){
        ...
        $field='id,nav_name,head_title,english,nav_color';
        $res=$this->N->where($condition)->field($field)->find();
        if($res!==false){
            return $res;
        }else{
    ...
    //获取热门导航信息
    public function getHotNav(){

        $condition['hot_nav']=1;

        $field='id,nav_name';
        $res=$this->N->where($condition)->order('sort asc')->field($field)->limit(10)->select();
